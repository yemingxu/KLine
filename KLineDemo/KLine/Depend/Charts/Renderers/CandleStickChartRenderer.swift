...
open class CandleStickChartRenderer: LineScatterCandleRadarRenderer
{
    ...
    fileprivate var _yMaxDrawPoint = CGPoint.zero;
    fileprivate var _yMinDrawPoint = CGPoint.zero;
    @objc open var yMinDrawPoint: CGPoint{return _yMinDrawPoint;}
    @objc open var yMaxDrawPoint: CGPoint{return _yMaxDrawPoint;}
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: ICandleChartDataSet)
    {
        ...
        for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
        {
            ...
            if showCandleBar
            {
                ...
                //绘制 最大最小值 提示
                if (j == indexOfYMin){
                    _yMinDrawPoint = _shadowPoints[2];
                }
                if (j == indexOfYMax){
                    _yMaxDrawPoint = _shadowPoints[0];
                }
